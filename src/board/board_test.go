...
func TestTileOrientation(t *testing.T) {
	tiles := loadTiles()

	_tile := tiles["CastleEndCap"]

	_tile.Placement.Orientation = 270
	td := _tile.Placement.TileDirection(directions.West)
	if _tile.Feature(td).Type != tile.Castle {
		t.Error("In Tile Space, the West Feature should be a castle")
	}

}

func BenchmarkVerifyPlacement(b *testing.B) {
	rand.Seed(0)
	...
	_tile := selectRandomTile(tiles)
	placements := board.PossibleTilePlacements(_tile)
	randomPlacement := placements[rand.Intn(len(placements))]
	b.ResetTimer()

	for n := 0; n < b.N; n++ {
		board.VerifyPlacement(_tile, randomPlacement)
	}
}
