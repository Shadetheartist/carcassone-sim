...
func (g *Game) updateRiverBuild() error {
	...
	if g.RiverDeck.Index == 0 {
		...
		g.Board.AddTile(&rt, tile.Placement{
			Position:    tile.Position{X: 30, Y: 30},
			Orientation: 0,
		})
...
func (g *Game) handleMouseInput() {
	...
	if ebiten.IsMouseButtonPressed(ebiten.MouseButtonRight) && g.HoveredPosition != g.SelectedPosition {
		...
		if t, exists := g.Board.Tiles[g.HoveredPosition]; exists {
			fmt.Println(t.Name)
		}

		if op, exists := g.Board.OpenPositions[g.HoveredPosition]; exists {
			for d, e := range op {
				dirName := directions.IntMap[directions.Direction(d)]
				fmt.Println(dirName, e)
			}
			fmt.Println()

			t, err := g.findTileForPos(g.SelectedPosition)
			if err != nil {
				fmt.Println("No tile fits this position")
			} else {
